...
class TestMyEntriesList:
    def test_entries_list_shows_only_users_entries(self, authenticated_client, user, another_user, entry):
        # entry уже создан для user (через фикстуру entry)
        # создаём запись другого пользователя
        from diary.models import Entry
        other_entry = Entry.objects.create(title='Чужая', content='...', author=another_user)
...
class TestEntryDetail:
    def test_detail_own_entry(self, authenticated_client, entry):
        response = authenticated_client.get(reverse('diary:entry_detail', args=[entry.pk]))
        assert response.status_code == 200
    ...
    def test_detail_another_user_404(self, authenticated_client, another_user):
        from diary.models import Entry
        other_entry = Entry.objects.create(title='Чужая', content='...', author=another_user)
        response = authenticated_client.get(reverse('diary:entry_detail', args=[other_entry.pk]))
...
class TestEntryEdit:
    def test_edit_get_form(self, authenticated_client, entry):
        response = authenticated_client.get(reverse('diary:entry_edit', args=[entry.pk]))
        assert response.status_code == 200
    ...
    def test_edit_post_valid(self, authenticated_client, entry):
        ...
        response = authenticated_client.post(reverse('diary:entry_edit', args=[entry.pk]), data)
        assert response.status_code == 302
    ...
    def test_edit_another_user_404(self, authenticated_client, another_user):
        from diary.models import Entry
        other_entry = Entry.objects.create(title='Чужая', content='...', author=another_user)
        response = authenticated_client.get(reverse('diary:entry_edit', args=[other_entry.pk]))
        assert response.status_code == 404


class TestEntryDelete:
    def test_delete_get_confirmation(self, authenticated_client, entry):
        response = authenticated_client.get(reverse('diary:entry_delete', args=[entry.pk]))
        assert response.status_code == 200
        assert response.context['entry'] == entry

    def test_delete_post(self, authenticated_client, entry):
        response = authenticated_client.post(reverse('diary:entry_delete', args=[entry.pk]))
        assert response.status_code == 302
    ...
    def test_delete_another_user_404(self, authenticated_client, another_user):
        from diary.models import Entry
        other_entry = Entry.objects.create(title='Чужая', content='...', author=another_user)
        response = authenticated_client.get(reverse('diary:entry_delete', args=[other_entry.pk]))
        assert response.status_code == 404